...
def save_project(path, name, structure, mesh_model, process_flow=None,
                 sweep=None):
    """v4 adds one optional key: "sweep" (the armed voltage sweep, as a
    plain dict or null).  Results are still never embedded -- a saved
    project is configuration, never in-progress or stale output."""
    data = {
...
def load_project(path):
    ...
    if version not in (2, 3, 4):
        raise UnsupportedProjectVersionError(
            ...
            f"(this build supports 2 and 3 (migrated) and {SCHEMA_VERSION})")
    ...
    return data["name"], structure, mesh_model, process_flow, sweep
