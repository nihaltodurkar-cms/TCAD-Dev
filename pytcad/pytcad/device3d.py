...
class Device3D:
    ...
    def _residual_jacobian(self, psi, n, p, voltages, theta=None,
                           ...
                           Jn_lag_z=None, Qheat_lag=None, current_rows_for=None):
        ...
        # ac3d.y_parameters) changes nothing; given, it only copies.
        Nx, Ny, Nz, N = self.Nx, self.Ny, self.Nz, self.N
        hx, hy, hz = self.hx, self.hy, self.hz
        dVx, dVy, dVz, dV = self.dVx, self.dVy, self.dVz, self.dV
        C = self.C
    ...
    def solve_bias(self, voltages=None, opts: NewtonOptions = None):
        ...
        # this cell (S3D, a cube diode: 44.56s direct -> 1.60s gmres at
        # 27,783 DOF). 2026-09-24: re-measured with F3D (the M26 tri-gate
        # FinFET), where gmres was ~5x SLOWER than direct; "auto" now
        # resolves to "mumps" (exact LU via PETSc/MUMPS) at or above
        # 6,591 DOF -- near the best on both shapes -- and to direct
        # below, or wherever no MUMPS-capable PETSc exists. The branch
        # below's per-iterate fallback to direct still applies. See
        # linsolve.select_auto / _AUTO_EVIDENCE for the numbers.
        resolved_linsolve, auto_reason = (
