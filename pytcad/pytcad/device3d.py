...
class Device3D:
    ...
    def _residual_jacobian_poisson(self, psi):
        ...
        for name, bc in self.bcs.items():
            if isinstance(bc, DirichletBC):
                kk = bc.k * Nx * Ny + bc.j * Nx + bc.i
                psi0, _, _ = _ohmic_values(
                    self.C[bc.k, bc.j, bc.i], self.nie_s[bc.k, bc.j, bc.i], 0.0, self.VT)
    ...
    def _residual_jacobian(self, psi, n, p, voltages):
        ...
        Fx_psi = dx / hx[None, None, :]
        Fy_psi = dy / hy[None, :, None]
        Fz_psi = dz / hz[:, None, None]
        ...
        # electron continuity row (comp 1), from Jn_x / Jn_y / Jn_z
        ...
        dJn_dpsiR_z = an_z * (n[1:, :, :] * dBp_z + n[:-1, :, :] * dBm_z)
        ...
        # hole continuity row (comp 2), from Jp_x / Jp_y / Jp_z
        ...
        dJp_dpsiR_z = ap_z * (p[1:, :, :] * dBm_z + p[:-1, :, :] * dBp_z)
        dJp_dp_L_z, dJp_dp_R_z = ap_z * Bp_z, -ap_z * Bm_z
        ...
        for name, bc in self.bcs.items():
            if isinstance(bc, DirichletBC):
                ...
                psi0, n0, p0 = _ohmic_values(
                    self.C[bc.k, bc.j, bc.i], self.nie_s[bc.k, bc.j, bc.i], V, self.VT)
    ...
    def solve_bias(self, voltages=None, opts: NewtonOptions = None):
        ...
        for name, bc in self.bcs.items():
            if isinstance(bc, DirichletBC):
                psi0, n0, p0 = _ohmic_values(
                    self.C[bc.k, bc.j, bc.i], self.nie_s[bc.k, bc.j, bc.i], bc.V, self.VT)
